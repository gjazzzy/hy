$(document).ready(function () {
   ...
   $(window).scroll(function () {
      if ($(window).scroll() < 200) {
         $(".card__recomends").css("display", "block")
      }
      else {
         $(".card__recomends").css("display", "none")
      }
   });

   // Hide modal 
   $('.pay, .adress__edit, .user, .burger, .card, .reg, .main__wrap, .rating, .orders, .relogin, .adress, .login').css('display', 'none');
   ...
   $(document).ready(function () {
      ...
      $(".left__sidebar-name").on("click", function (event) {
         event.preventDefault(); // Отменяем стандартное поведение ссылки
         $(".left__sidebar-item").toggle(); // Переключаем видимость выпадающего списка
      });

      // Закрыть выпадающий список при клике вне его
      $(document).on("click", function (event) {
         if (!$(event.target).closest('.left__sidebar-item').length && !$(event.target).is('.left__sidebar-name')) {
            $(".left__sidebar-item").hide(); // Скрываем выпадающий список
         }
      });
   });
